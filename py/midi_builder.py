...
def make_midi(filename, type):
    ...
    if type == MusicBuildType.DROPS:
        ...
        beats = add_chord_progression(chord_progression, octave_start=OCTAVE, note_duration=[2,2,2,2,4,4,8], skip_random=skip_random, group_chord=True)
        channels.append(mid.Channel(beats=beats)) 

        OCTAVE = 4
        beats = add_chord_progression(chord_progression, octave_start=OCTAVE, note_duration=[0.5,0.5,1,2], skip_random=skip_random, group_chord=False, randomize=True)
        channels.append(mid.Channel(beats=beats)) 
